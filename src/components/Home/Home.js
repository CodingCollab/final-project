import React, { Component } from "react";
import { Card, Button, CardImg, CardTitle, CardText, CardGroup,
  CardBody } from 'reactstrap';

const Home = (props) => {
  return (
    <div>
    <CardGroup>
      <Card>
        <CardImg top width="100%" object-fit="cover" src="https://www.mxcursos.com/image/cache/data/Thumbs2017/javascript-cr-292x175.png" alt="Card image cap" />
        <CardBody>
          <CardTitle>Javascript</CardTitle>
          <CardText>Row 1 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View Javascript Tasks</Button>
        </CardBody>
      </Card>
      <Card>
        <CardImg top width="100%" object-fit="cover" src="data:image/png;base64," alt="Card image cap" />
        <CardBody>
          <CardTitle>PHP</CardTitle>
          <CardText>Row 1 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View PHP Tasks</Button>
        </CardBody>
      </Card>
      <Card>
        <CardImg top width="100%" object-fit="cover" src="https://www2.bc.edu/tim-townsend/images/html_css.png" alt="Card image cap" />
        <CardBody>
          <CardTitle>HTML/CSS</CardTitle>
          <CardText>Row 1 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View HTML/CSS Tasks</Button>
        </CardBody>
      </Card>
    </CardGroup>
    
    <br></br>

    <CardGroup>
      <Card>
        <CardImg top width="100%" src="https://ptpimg.me/67j0yz.png" alt="Card image cap" />
        <CardBody>
          <CardTitle>Node</CardTitle>
          <CardText>Row 2 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View Node Tasks</Button>
        </CardBody>
      </Card>
      <Card>
        <CardImg top width="100%" src="https://ptpimg.me/jx6w82.png" alt="Card image cap" />
        <CardBody>
          <CardTitle>React</CardTitle>
          <CardText>Row 2 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View React Tasks</Button>
        </CardBody>
      </Card>
      <Card>
        <CardImg top width="100%" src="https://ptpimg.me/v6819k.png" alt="Card image cap" />
        <CardBody>
          <CardTitle>MySQL</CardTitle>
          <CardText>Row 2 card has supporting text below as a natural lead-in to additional content.</CardText>
          <Button color="primary">View MySQL Tasks</Button>
        </CardBody>
      </Card>
    </CardGroup>
    </div>
  );
};

export default Home;